...
export default function ProbeDisplay({ label, data, targetTempC, unit }: Props) {
  const surface: React.CSSProperties = {
    background: 'var(--surface)', borderRadius: '12px', padding: '1.5rem', flex: 1,
  }

  if (!data) return (
    <div style={surface}>
      <div style={{ color: 'var(--muted)', fontSize: '0.85rem', marginBottom: '0.5rem' }}>{label}</div>
  ...
  return (
    <div style={surface}>
      ...
      <div style={{ fontSize: '3rem', fontWeight: 'bold', color: done ? 'var(--green)' : 'var(--accent)' }}>
      ...
      {targetTempC && (
        <>
          ...
          <div style={{ marginTop: '0.5rem', height: '6px', background: 'var(--border)', borderRadius: '3px' }}>
            <div style={{
              width: `${pct}%`, height: '100%', borderRadius: '3px', transition: 'width 0.5s',
